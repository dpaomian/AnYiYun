...
// 背景色
#define GlobelBgColor HJCColor(178, 178, 178)
// 分割线颜色
#define GlobelSeparatorColor HJCColor(204, 204, 204)
// 普通状态下的图片
...
- (instancetype)initWithDelegate:(id<HJCActionSheetDelegate>)delegate andMessageTitle:(NSString *)messageTitle CancelTitle:(NSString *)cancelTitle OtherTitles:(NSString *)otherTitles, ...
{
    ...
    {
        messageHeight = 40;
        [self setupMessageWithTitle:messageTitle];
    }
    ...
    sheetViewF.size.height = BtnHeight * _tag + Margin+messageHeight;
...
- (void)setupMessageWithTitle:(NSString *)title{
    
    UILabel   *messageLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, ScreenWidth, messageHeight)];
    messageLabel.backgroundColor = [UIColor whiteColor];
    messageLabel.textColor = [UIColor grayColor];
    messageLabel.numberOfLines = 2;
    messageLabel.text = title;
    messageLabel.font = HeitiLight(12);
    messageLabel.textAlignment = NSTextAlignmentCenter;
    [self.sheetView addSubview:messageLabel];
...
- (void)setupBtnWithTitle:(NSString *)title{
    ...
    // 创建按钮
    UIButton *btn = [[UIButton alloc] initWithFrame:CGRectMake(0, BtnHeight * (_tag - 1)+messageHeight , ScreenWidth, BtnHeight)];
    [btn setBackgroundImage:normalImage forState:UIControlStateNormal];
    [btn setBackgroundImage:highImage forState:UIControlStateHighlighted];
